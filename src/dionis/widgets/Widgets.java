...
public final class Widgets {
	...
	public static abstract class IntegerTextEntryHandler implements
			FocusListener, VerifyListener, TraverseListener {
		private static final Pattern NEGATIVE_PATTERN = Pattern
				.compile("\\-?|\\-?\\d+");
		private static final Pattern NON_NEGATIVE_PATTERN = Pattern
				.compile("\\d+");
		...
		public IntegerTextEntryHandler(Text txt, boolean allowNegatives,
				int limit) {
			this.txt = txt;
	...
	public static abstract class RealNumberTextEntryHandler implements
			FocusListener, VerifyListener, TraverseListener {
		private static final Pattern NON_NEGATIVE_PATTERN = Pattern
				.compile("\\d*\\.?\\d*");
		private static final Pattern NEGATIVE_PATTERN = Pattern
		...
		public RealNumberTextEntryHandler(Text txt, boolean allowNegatives,
				int limit) {
			this.txt = txt;
			txt.addFocusListener(this);
			txt.addVerifyListener(this);
	 ...
	 */
	public static GridData positionGridItem(Control control,
			boolean grabHExcess, boolean grabVExcess, int halign, int valign,
			int hspan) {
	 ...
	 * @return Return grid data reference
	 */
	public static GridData positionGridItem(final Control control,
			final boolean grabHExcess, final boolean grabVExcess,
	 ...
	 */
	public static GridData positionGridItem(Control control,
			boolean grabHExcess, int halign, int valign, int hspan) {
		final GridData gd = new GridData();
		gd.grabExcessHorizontalSpace = grabHExcess;
		gd.horizontalAlignment = halign;
		gd.horizontalSpan = hspan;
	 ...
	 */
	public static GridData positionGridItem(final Control control,
			final boolean grabHExcess, final int halign, final int valign) {
		final GridData gd = new GridData();
		gd.grabExcessHorizontalSpace = grabHExcess;
		gd.horizontalAlignment = halign;
		gd.verticalAlignment = valign;
	 ...
	 */
	public static GridData positionGridItem(Control control,
			boolean grabHExcess, int halign, int valign, int hspan, int vspan) {
		final GridData gd = new GridData();
		gd.grabExcessHorizontalSpace = grabHExcess;
		gd.horizontalAlignment = halign;
		gd.horizontalSpan = hspan;
	...
	public static GridData positionGridItem(Control control,
			boolean grabHExcess, boolean grabVExcess, int halign, int valign,
			int hspan, int vspan, int width, int height) {
		final GridData gd = new GridData();
		gd.grabExcessHorizontalSpace = grabHExcess;
		gd.grabExcessVerticalSpace = grabVExcess;
		gd.horizontalAlignment = halign;
	...
	public static Text createTxt(Composite comp, final int style,
			String defaultTxt, int limit) {
	 ...
	 */
	public static Text createNumericTxt(Composite comp, String defaultTxt,
			int limit) {
	...
	public static Text createDecimalText(Composite comp, String defaultTxt,
			int min, int max, int limit, int style) {
		final Text txtBox = new Text(comp, style);
		txtBox.setTextLimit(limit);
		txtBox.setText(defaultTxt);
		txtBox.addVerifyListener(new VerifyListener() {
	...
	public static void setGridLayout(final Composite item, int numColumns,
			int columnSpacing) {
	...
	public static void setRowLayout(final Composite item, final int style,
			final boolean wrap, final boolean pack) {
	...
	public static void setGridLayout(Composite item, int numColumns,
			int columnSpacing, boolean equal, int hpad) {
		final GridLayout layout = new GridLayout();
		layout.numColumns = numColumns;
		layout.horizontalSpacing = columnSpacing;
	...
	public static GridLayout setGridLayout(Composite item, int numColumns,
			int columnSpacing, int verticalSpacing, boolean equal, int hpad) {
		final GridLayout layout = new GridLayout();
		layout.numColumns = numColumns;
		layout.horizontalSpacing = columnSpacing;
		layout.makeColumnsEqualWidth = equal;
	...
	public static Group createRadioGrp(final String txt, final int columns,
			final Object[] items) {
		final Group grp = new Group(targetContainer, SWT.NONE);
		grp.setText(txt);
	...
	public static Group createRadioGrp(Composite comp, String txt, int columns,
			Object[] items) {
	...
	/**
	 * Used in form layouts. Attaches the specified control to the specified
	 * edge of the item's parent
	 * 
	 * @param control
	 * @param edge
	 * @param percent
	 * @param offset
	 */
	public static void attachToParent(final Control control, final int edge,
			final int percent, final int offset) {
		attachToParent(control, edge, percent, offset, SWT.DEFAULT, SWT.DEFAULT);
	}

	public static void attachToParent(final Control control, final int edge,
			final int percent, final int offset, int width, int height) {
	 ...
	 * 
	 * @param control
	 *            the parimary control that will be attached to another control
	 * @param itemToAttachTo
	 *            the secondary control that the primary control will attach to
	 * @param edge
	 *            the primary control's edge to attach
	 * @param itemToAttachToEdge
	 *            the secondary control's edge
	 * @param offset
	 *            the number of pixels between the two controls' edges
	 */
	public static void attachToControl(final Control control,
			final Control itemToAttachTo, final int edge,
			final int itemToAttachToEdge, final int offset) {
		attachToControl(control, itemToAttachTo, edge, itemToAttachToEdge,
				offset, SWT.DEFAULT, SWT.DEFAULT);
	}
	...
	public static void attachToControl(final Control control,
			final Control itemToAttachTo, final int edge,
			final int itemToAttachToEdge, final int offset, final int width,
			final int height) {
		final Object ld = control.getLayoutData();
		final FormData fd = (ld != null) ? (FormData) ld : new FormData();
		switch (edge) {
		case SWT.LEFT:
			fd.left = new FormAttachment(itemToAttachTo, offset,
					itemToAttachToEdge);
			break;
		case SWT.RIGHT:
			fd.right = new FormAttachment(itemToAttachTo, offset,
					itemToAttachToEdge);
			break;
		case SWT.TOP:
			fd.top = new FormAttachment(itemToAttachTo, offset,
					itemToAttachToEdge);
			break;
		case SWT.BOTTOM:
			fd.bottom = new FormAttachment(itemToAttachTo, offset,
					itemToAttachToEdge);
			break;
		default:
			throw new IllegalArgumentException("invalid edge specified");
	...
	/**
	 * disposes all the supplied widgets if they are not null and not already
	 * disposed
	 * 
	 * @param <T>
	 * @param widgets
