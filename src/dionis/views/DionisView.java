...
import dionis.actions.filters.AddExtendedItemAction;
import dionis.actions.filters.AddNewBlankFilterAction;
import dionis.actions.filters.AddSheduleItemAction;
import dionis.actions.filters.AddStandardItemAction;
import dionis.actions.filters.AddTemplateItemAction;
import dionis.actions.filters.BlockFilterItemAction;
import dionis.actions.filters.ChangeFilterItemAction;
import dionis.actions.filters.ChangeNameAction;
import dionis.actions.filters.DeleteFilterAction;
import dionis.actions.filters.MakeTemplateItemAction;
import dionis.actions.filters.RemoveFilterItemAction;
import dionis.actions.filters.UnblockFilterItemAction;
					...
					if (!table.getTable().getSelection()[0].getText().contains(
						...
						if (dialog.open() == Window.OK) {
							Display.getDefault().asyncExec(new Runnable() {
								@Override
								public void run() {
									table.setInput(PortsModel.getInstance()
											.getAllPorts());
								}
							});
						}
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (dialog.open() == Window.OK) {
					Display.getDefault().asyncExec(new Runnable() {
						@Override
						public void run() {
							tableViewer_1.setInput(AddressModel.getInstance()
									.getAllAddressArray());
						}
					});
				}
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (dialog.open() == Window.OK) {
					...
					Display.getDefault().asyncExec(new Runnable() {
						...
						public void run() {
							// обновление данных для таблицы
							interfaceTableViewer.setInput(InterfaceModel
		...
		pasteItem.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				Display.getDefault().asyncExec(new Runnable() {
					@Override
					public void run() {
						tunnelTableViewer.setInput(TunnelModel.getInstance()
								.getDataArray());
					}
				});
			}
